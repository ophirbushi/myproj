...
export class MergeDecisionsComponent {
  @Input() state!: State
  @Output() postInput = new EventEmitter<MergeDecision[]>()
  ...
  postMergeDecide() {
    const mergeDecisions: MergeDecision[] = Object.entries(this.mergeDecisions)
      .map(([key, value]) => ({ hotelIndex: +key, sell: value.sell, convert: value.convert }))
      .filter((decision) => decision.sell > 0 || decision.convert > 0)
      .filter(decision => hotelExistsOnBoard(this.state, decision.hotelIndex))
    this.postInput.emit(mergeDecisions)
  }
}
